...
func main() {
	...
	for i := range test.Grlay {
		test.Grlay[i].Init(float64(i%5)+3.0, rand.Float64(), rand.Float64(), spacesize, 0.8, 0.6, 0.15)
	}
	...
	plt.Save(str+"/figure/grid_cell", filename)
	...
	levels = []float64{0, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3}
	for i := 0; i < len(data); {
		for j := 0; j < len(data[0]); {
			...
			plt.Save(str+"/figure/grid_cell/ip", filename)
			j +=2
		}
		i+=2
	}
