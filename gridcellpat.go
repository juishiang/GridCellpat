...
func main() {
	placedevsizeS := []string{"010", "015", "020"}
	placedevsizeF := []float64{0.1, 0.15, 0.2}
	...
	for idx, placedevsizeFvalue := range placedevsizeF {
		for Ridx, tr := range SpRan {
			test.Init(neunum)
			str, _ := os.Getwd()
			str = str + "/figure/" + placedevsizeS[idx] + SpRanS[Ridx]
			csvname := gi.FileName(placedevsizeS[idx] + strconv.FormatBool(tr) + ".csv")
			for i := range test.Grlay {
				if tr {
					test.Grlay[i].Init(float64(i%5)+3.0, rand.Float64(), rand.Float64(), spacesize, 0.8, 0.6, placedevsizeFvalue)
				} else {
					test.Grlay[i].Init(float64(i%5)+3.0, float64(i%5)/5.0, float64(i/25)/5.0, spacesize, 0.8, 0.6, placedevsizeFvalue)
				}
			}
